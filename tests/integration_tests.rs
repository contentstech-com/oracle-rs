...
mod lob_bind_tests {
    ...
    async fn test_temp_lob_insert() {
        use oracle_rs::{LobData, LobValue, OracleType};
        ...
        match insert_result {
            Ok(_) => {
                let verify = conn
                    ...
                    .await
                    .expect("Failed to query inserted temp LOB");
            ...
            Err(err) => {
                ...
                cleanup_conn
                    .execute("DELETE FROM temp_lob_insert_test WHERE id = 100", &[])
                    .await
                    .ok();
...
mod lob_workaround_tests {
    use super::*;
    use oracle_rs::{BindParam, LobData, Value};
    ...
    async fn test_dbms_lob_returning_into() {
        ...
        conn.execute_plsql(&plsql, &[BindParam::input(test_id.into())])
            .await
        ...
        // Verify
        let verify_sql = format!("SELECT content FROM {} WHERE id = {}", table_name, test_id);
        let rows = conn.query(&verify_sql, &[]).await.expect("Failed to query");
        ...
        // Clean up
        conn.execute(&format!("DROP TABLE {} PURGE", table_name), &[])
            .await
            .ok();
        conn.commit().await.expect("Failed to commit cleanup");
        conn.close().await.expect("Failed to close");
        ...
        loop {
            for row in &result.rows {
                let n = match row.get(0) {
                    Some(Value::String(s)) => s.parse::<u64>().unwrap_or_else(|_| {
                        panic!("Row {}: non-numeric value {:?}", next_expected, s)
                    }),
                    Some(v) => v.as_i64().unwrap_or_else(|| {
                        panic!("Row {}: unexpected value {:?}", next_expected, v)
                    }) as u64,
